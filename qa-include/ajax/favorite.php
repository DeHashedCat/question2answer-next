...
$setfavorite = (int)qa_post_text('favorite');
...
$allowedEntityTypes = array(QA_ENTITY_QUESTION, QA_ENTITY_USER, QA_ENTITY_TAG, QA_ENTITY_CATEGORY);
if (!isset($userid) ||
	!in_array($entitytype, $allowedEntityTypes, true) ||
	!ctype_digit((string)$entityid) ||
	(int)$entityid <= 0 ||
	strlen((string)$entityid) > 18) {
	echo "QA_AJAX_RESPONSE\n0\n" . qa_lang('misc/form_security_reload');
	return;
}
...
qa_user_favorite_set($userid, qa_get_logged_in_handle(), $cookieid, $entitytype, (int)$entityid, $setfavorite);

$favoriteform = qa_favorite_form($entitytype, (int)$entityid, $setfavorite, qa_lang($setfavorite ? 'main/remove_favorites' : 'main/add_favorites'));
